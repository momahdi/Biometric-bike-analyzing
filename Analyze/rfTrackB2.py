...
def main():
    ...
    tr_df["y"], te_df["y"] = le.transform(tr_df["participant_id"]), le.transform(
        te_df["participant_id"]
    )
    ...
    fi_raw = dict(
        zip(Xtr.columns[: len(rf.feature_importances_)], rf.feature_importances_)
    )
    fi = pd.Series({f: fi_raw.get(f, 0.0) for f in FEATURES}).sort_values(
        ascending=False
    )
    ...
    plt.title(f"Random-Forest confusion matrix  |  Train {TRAIN_DAY} → Test {TEST_DAY}")
    plt.tight_layout()
    plt.show()
